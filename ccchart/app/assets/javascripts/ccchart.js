  ...
  return {
    ...
    preProcessing: function (op, callback) {
      ...
      */
      ...
      //WebSocket詳細デバッグ用フラグ
      this.wsDbg = this.op.config.wsDbg || this.gcf.wsDbg || false;
      //WebSocket接続/切断等info出力用フラグ
      this.wsInfo = this.op.config.wsInfo || this.gcf.wsInfo || false;
      ...
      for(var i = 0;i< this.data.length; i++){ 
        ...
        _data[i][j] = parseFloat(this.data[i][j]||0)//nullなどは0に
        }
      }
      this.data = _data;
      ...
      //X目盛を小数点有りにするか？
      this.xScaleDecimal = 
        this.op.config.xScaleDecimal || this.gcf.xScaleDecimal || false;
      //Y目盛を小数点有りにするか？
      this.yScaleDecimal = 
        this.op.config.yScaleDecimal || this.gcf.yScaleDecimal || false;
    ...
    drawVals: function () {
      ...
      this.ctx.save();
      var xOffset = 
        this.valXOffset = this.op.config.valXOffset || this.gcf.valXOffset || 0;
      var yOffset = 
     ...
     // var sumCols = this.util.getSumCols(this);
      ...
      for (var k = 0; k < this.dataRowLen; k++) {
        ...
        for (var l = 0; l < this.data[k].length; l++) {
          ...
          var val = (this.percentVal === 'yes')? percent : this.data[k][l];
          this.ctx.beginPath();
